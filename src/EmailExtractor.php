...
class EmailExtractor {
  ...
  const EMAIL_NON_VALID_CHARACTERS = [',', '\n', '\'', ':'];
  const SEPARATOR = ' ';

  function getEmailsFromString($input) {
    #replace all white spaces by a normal white space
    $input = preg_replace('/\s+/', ' ', $input);
    #clean the input from non valid characters
    $clean_input = str_replace(self::EMAIL_NON_VALID_CHARACTERS, self::SEPARATOR, $input);
    #split the input, each value need to have only one email
    $input_list = explode(self::SEPARATOR, $clean_input);
    return preg_grep(self::EMAIL_REGEXP, $input_list);
  }
}
